...
import { getLake } from "../graphql/queries";
import { updateLake } from "../graphql/mutations";
...
export default function LakeUpdateForm(props) {
  const {
    id: idProp,
    lake: lakeModelProp,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = lakeRecord
      ? { ...initialValues, ...lakeRecord }
      : initialValues;
  ...
  const [lakeRecord, setLakeRecord] = React.useState(lakeModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getLake.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getLake
        : lakeModelProp;
      setLakeRecord(record);
    };
    queryData();
  }, [idProp, lakeModelProp]);
  React.useEffect(resetStateValues, [lakeRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateLake.replaceAll("__typename", ""),
            variables: {
              input: {
                id: lakeRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "LakeUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || lakeModelProp)}
